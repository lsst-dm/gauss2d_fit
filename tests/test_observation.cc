...
TEST_CASE("Observation") {
    ...
    CHECK(gauss2d::images_compatible<double, Image, double, Image>(
        observation->get_image(), observation->get_sigma_inverse()));
    CHECK(gauss2d::images_compatible<double, Image, bool, Mask>(
        observation->get_image(), observation->get_mask_inverse()));
