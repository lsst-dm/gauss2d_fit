...
TEST_CASE("LinearSersicMixInterpolator")
{
    ...
    for(const auto order : orders) {
        ...
        CHECK_THROWS_AS(interpolator.get_integralsizes(
            interpolator.sersicindex_max + 1e-10),
        ...
        for(const double factor : {0., 0.5, 1.})
        {
            ...
            for(size_t ord = 0; ord < order; ++ord) {
                ...
                msg = "sigma[" + std::to_string(ord) + "] deriv,findiff="
                    + std::to_string(deriv) + "," + std::to_string(findiff) + " result="
                    + close_sigma.str();
