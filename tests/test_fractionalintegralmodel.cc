...
TEST_CASE("FractionalIntegralModel") {
    ...
    CHECK_EQ(frac->at(channel)->get_value(), 0);
    CHECK_EQ(frac->get_integral_remainder(channel), value);

    // Set the fraction to 100% (previously 0)
    frac->at(channel)->set_value(1.);
    CHECK_EQ(frac->get_integral(channel), value);
    ...
    CHECK_EQ(frac->get_integral(channel), frac1 * value);
    CHECK_EQ(frac->get_integral_remainder(channel), (1. - frac1) * value);

    // Check that the frac gives both parameters (parent's integral and its own fraction)
    g2f::ParamCRefs params{};
    CHECK_EQ(frac->get_parameters_const(params).size(), 2);
    ...
    CHECK_EQ(frac2->get_integral(channel), (1 - frac1) * value);
