...
PriorEvaluation GaussianPrior::evaluate(bool calc_jacobians, bool normalize) const {
    double residual = ((_transformed ? _param->get_value_transformed() : _param->get_value()) - _mean)/_stddev;
    if(!std::isfinite(residual)) {
        throw std::runtime_error(this->str() + ".evaluate() got non-finite residual=" + std::to_string(residual));
    }
...
double GaussianPrior::get_loglike_const_term() const {
...
std::string GaussianPrior::str() const {
    return "GaussianPrior(param=" + _param->str() + ", mean=" + std::to_string(_mean) + ", stddev="
        + std::to_string(_stddev) + ", transformed=" + std::to_string(_transformed) + ")";
}

GaussianPrior::GaussianPrior(
    std::shared_ptr<const ParamBase> param, double mean, double stddev, bool transformed
    ...
    if(!std::isfinite(loglike)) {
        throw std::invalid_argument(this->str() + " has non-finite loglike=" + std::to_string(loglike) + " on init");
    }
}
