...
std::vector<IntegralSize> LinearSersicMixInterpolator::get_integralsizes(double sersicindex) const {
    ...
    if(sersicindex == sersicindex_min) return knots[0].values;
    else if(sersicindex == sersicindex_max) return knots.back().values;
    ...
    if(!((frac_low >= 0 ) && (frac_low <= 1))) {
        throw std::logic_error(
            "Got invalid frac_low=" + std::to_string(frac_low) + " with n, lo, hi="
            + std::to_string(sersicindex) + "," + std::to_string(low.sersicindex) + ","
            + std::to_string(high.sersicindex)
        );
    }
    ...
    for(size_t i = 0; i < _order; ++i) {
        result.push_back({
            frac_low*low.values[i].integral + frac_high*high.values[i].integral,
