...
ParamCRefs GaussianComponent::_get_parameters_grad(const Channel & channel) const {
    ...
    if(!(n_params_int <= N_PARAMS_INTEGRAL_MAX)) {
        ...
        this->str() + "->_integralmodel->get_parameters_const(channel=" + channel.str() + ").size())="
            + std::to_string(params_int.size()) + "!<=" + std::to_string(N_PARAMS_INTEGRAL_MAX)
        );
    }
...
) const
{
    ...
    if(!(n_factors <= N_PARAMS_EXTRA_INTEGRAL_MAX)) {
        throw std::runtime_error(_integralmodel->str() + ".get_integral_derivative_factors("
            + channel.str() + ").size()=" + std::to_string(n_factors) + "!<=" 
...
void GaussianComponent::set_grad_param_factors(
    ...
    for(size_t idx_param = 0; idx_param < N_PARAMS_GAUSS2D; ++idx_param) {
        ...
        if(param.get_fixed()) {
            ...
            if(deriv == 0) {
                throw std::runtime_error("Param[idx=" + std::to_string(idx_param) + "="
